...
type
  ...
  public
     ...
     function    MovePointer(tp : pointer; ammount : integer ) : pointer;
...
type
  ...
  public
    ...
    procedure SaveMatrixDataRawBin(filenameOut : string) ;  // only saves F_data
    ...
    function  GetTotalRowsColsFromString(range :string; rowcolList : TMemoryStream) : integer ; // rowcolList Stream is filled with zero based list of column/row numbers
...
function TMaxMin.MovePointer(tp : pointer; ammount : integer ) : pointer;
begin
   asm
   MOV     EAX,tp
   MOV     EDX,ammount
...
//////////////////////////////////////////////////////////////////////////////////////
function TMatrix.MovePointer(tp : pointer; ammount : integer ) : pointer;
begin
   asm
   MOV     EAX,tp
   MOV     EDX,ammount
...
procedure TMatrix.ClearData(singleOrDouble : integer) ;  // = 1 for single, 2 for double
begin
  SDPrec := singleOrDouble * 4 ;
  complexMat := 1 ;
...
begin
   self.ClearData(self.SDPrec div 4) ;
...
begin
  ...
  begin
     messagedlg('TMatrix.MakeComplex() Error: Real and Imginary matrix size does not match. Have to exit' ,mtError,[mbOK],0) ;
     exit ;
  end ;
...
procedure TMatrix.Average() ;      // can deal with complex
var
   t1, t2 : integer ;
   p1, p2 : pointer ;
   s1 : single ;
...
begin
   GetMem(p1, SDPrec) ;
   GetMem(p2, SDPrec) ;
   F_MAverage.SetSize(numCols*SDPrec) ;
   Zero(F_MAverage) ;
   ...
   for t2 := 1 to (numCols * complexMat) do
   begin
        F_MAverage.Read(p1^,SDPrec) ;
        F_MAverage.Seek(-SDPrec,soFromCurrent) ;
        if SDPrec = 4 then                      // single precision
           single(p1^) := single(p1^) * s1
        else                                    // double precision
           double(p1^) := double(p1^) * d1 ;
        F_MAverage.Write(p1^,SDPrec) ;
   end ;
...
try
  ...
  messagedlg('TMatrix.AverageSmooth Error: number of points to use has to be an odd number' ,mtError,[mbOK],0) ;
...
begin
  ...
  do
     messagedlg('TMatrix.MultiplyByScalar() Error: Overflow error' ,mtError,[mbOK],0) ;
  end ;
  self.F_Mdata.Seek(0,soFromBeginning) ;
...
begin
  ...
  messagedlg('TMatrix.AddOrSubtractMatrix Error: matricies not same size' ,mtError,[mbOK],0) ;
...
//  memStrIn.Seek(0, SoFromBeginning) ;
  ...
  begin
    messagedlg('EZeroDivide',mtError,[mbOK],0) ;
    result := 1 ;
  end ;
  end ;
  end ;
...
begin
  ...
  begin
     messagedlg('AddRowToEndOfData() Error: number of columns of input does not match number of columns of TMatrix being added to' ,mtError,[mbOK],0) ;
  end ;
...
begin
   ...
   begin
      messagedlg('AddRowsToTMatrix() Error: number of columns of input does not match number of columns of TMatrix being added to' ,mtError,[mbOK],0) ;
   end ;
...
begin
  ...
  begin
     messagedlg('AddColToEndOfData() Error: number of rows of input does not match number of rows of TMatrix' ,mtError,[mbOK],0) ;
  end ;
  FreeMem(p1) ;
  tMemStr.Free ;
end ;
...
begin
   ...
   begin
     messagedlg('TMatrix.AddColumnsToMatrix() Error: Incorrect number of rows in column data.' ,mtError,[mbOK],0) ;
     exit ;
   end ;
   ...
   tMat := TMatrix.Create(self.SDPrec div 4) ;
...
//  else
  ...
  begin
    ...
    begin
      result := false ;
      messagedlg('TMatrix.CheckRangeInput() Error: "'+ rangeIn + '" is not a valid numeric input',mtError,[mbOK],0) ;
      exit ;
    end;
    end ; // except
  ...
  begin
    result := false ;
    messagedlg('TMatrix.CheckRangeInput() Error: "'+ rangeIn + '" is not a valid input',mtError,[mbOK],0) ;
    exit ;
  end;
  ...
  begin
     ...
     begin
        ...
        begin
          result := false ;
          messagedlg('TMatrix.CheckRangeInput() Error: "'+copy(Range,1,pos_dash-1)+'" is not a valid numeric input' ,mtError,[mbOK],0) ;
          exit ;
        end;
        end ; // try/except
     ...
     begin
        ...
        begin
          result := false ;
          messagedlg('TMatrix.CheckRangeInput() Error: "'+copy(Range,1,pos_comma-1)+'" is not a valid numeric input' ,mtError,[mbOK],0) ;
          exit ;
        end;
        end ; // try/except
...
begin
   ...
   FreeMem(p1) ;
     ...
     messagedlg('Error: TMatrix.FetchDataFromTMatrix(): Input data range not valid' ,mtError,[mbOK],0) ;
end ;
...
begin
   ...
   reducedM :=  TMatrix.Create(SDPrec div 4) ; // temporary storage
...
begin
    ...
    begin
      ...
      if s_denom <> 0.0 then
        ...
        messagedlg('Error: TMatrix.GetSlope(): Slope is infinite (div by zero error)' ,mtError,[mbOK],0) ;
      ...
      messagedlg('Error: TMatrix.GetSlope(): Slope can only be determined for data in 2 columns' ,mtError,[mbOK],0) ;
end ;
...
begin
    ...
    begin
      ...
      messagedlg('Error: TMatrix.GetIntercept(): Intercept can only be determined for data in 2 columns' ,mtinformation,[mbOK],0) ;
end ;
...
begin
    ...
    begin
      ...
      if single(x_ave^) <> 0.0 then
        ...
        messagedlg('Error: TMatrix.GetSlopeForcedThroughZero(): Slope is infinite (div by zero error)' ,mtError,[mbOK],0) ;

      FreeMem(x_ave) ;
      FreeMem(y_ave) ;
    end
    else
      messagedlg('Error: TMatrix.GetSlopeForcedThroughZero(): Slope can only be determined for data in 2 columns' ,mtError,[mbOK],0) ;
end ;
...
begin
   ...
   begin
     messagedlg('TMatrix.CopyUpperToLower error: Matrix is not square ' ,mtError,[mbOK],0) ;
     exit ;
   end ;
...
begin
   ...
   begin
     messagedlg('TMatrix.LoadMatrixDataFromBinFile() Warning: Could not determine if data was complex so set as non-complex' ,mtError,[mbOK],0) ;
     self.complexMat := 1
   end ;
...
procedure TMatrix.SaveMatrixDataRawBin(filenameOut : string) ;  // save as binary file
var
  t1 : integer ;
begin
   self.F_Mdata.SaveToFile(filenameOut) ;
...
begin
    ...
    begin
       ...
       begin
         messagedlg('No delimeter of type ''+inDelimeter+'' found' ,mtError,[mbOK],0) ;
         result := '-1' ;
         exit ;
       end ;
    end ;
...
begin
  ...
  begin
    ...
    begin
     ...
     begin
      messagedlg('TMatrix.LoadMatrixDataFromTxtFile Error:'+#13+'File: '+extractFilename(filenameIn)+#13+' does not exist!' ,mtError,[mbOK],0) ;
      result := false ;
      exit ;
     end ;
...
//             if FloatToStrF(single(p1^),ffgeneral,5,3) = 'NAN' then  single(p1^) := 5 ;   // addedxxx
  ...
  if SDPrec = 4 then
  begin
    retValue := TMaxMin.Create(SDPrec div 4) ;
