...
//     ColorDepth  - the actual color depth you want, this is only useful for bitmap contexts
//     StencilBits - determines the size of the stencil buffer
//   RESULT: the newly created context or 0 if setup faild
//
...
//   Determines which extensions for the current rendering context are available
...
//
//  Contact:  Lischke@imib.med.tu-dresden.de
//
//  last change : 05. December 1997
//
//------------------------------------------------------------------------------
...
const // ********** GL generic constants **********
      ...
      //GL_MAX_ELEMENTS_VERTICES           = // not yet defined; // GL 1.2
      //GL_MAX_ELEMENTS_INDICES            = // not yet defined; // GL 1.2
      //GL_RESCALE_NORMAL                  = // not yet defined; // GL 1.2
...
const // additional constants, which are not defined in Windows.pas
...
//------------------------------------------------------------------------------
    ...
    // Extension functions
    glAddSwapHintRectWIN : procedure(x, y: GLint; width, height: GLsizei);
...
begin
  ...
  if Separator > 0 then MajorVersion:=StrToInt(Copy(Buffer,1,Separator-1))
                   else MajorVersion:=0;
  Delete(Buffer,1,Separator);
  Separator:=Pos('.',Buffer);
  if Separator > 0 then MinorVersion:=StrToInt(Copy(Buffer,1,Separator-1))
                   else MinorVersion:=0;
  ...
  begin
    ...
    if Separator > 0 then MinorVersion:=StrToInt(Copy(Buffer,1,Separator-1))
      ...
      if Length(Buffer) > 0 then MinorVersion:=StrToInt(Buffer)
                            else MinorVersion:=0;
    if (MajorVersion > 0) and (MinorVersion > 0) then
    begin
  ...
  if Pos('GL_EXT_abgr',Buffer) > 0 then GL_EXT_abgr:=True
                                   else GL_EXT_abgr:=False;
  if Pos('GL_EXT_bgra',Buffer) > 0 then GL_EXT_bgra:=True
                                   else GL_EXT_bgra:=False;
  if Pos('GL_EXT_packed_pixels',Buffer) > 0 then GL_EXT_packed_pixels:=True
                                            else GL_EXT_packed_pixels:=False;
  if Pos('GL_EXT_paletted_texture',Buffer) > 0 then GL_EXT_paletted_texture:=True
                                               else GL_EXT_paletted_texture:=False;
  if Pos('GL_EXT_vertex_array',Buffer) > 0 then GL_EXT_vertex_array:=True
                                           else GL_EXT_vertex_array:=False;
  if Pos('GL_SGI_compiled_vertex_array',Buffer) > 0 then GL_SGI_compiled_vertex_array:=True
                                                    else GL_SGI_compiled_vertex_array:=False;
  if Pos('GL_SGI_cull_vertex',Buffer) > 0 then GL_SGI_cull_vertex:=True
                                          else GL_SGI_cull_vertex:=False;
  if Pos('GL_SGI_index_array_formats',Buffer) > 0 then GL_SGI_index_array_formats:=True
                                                  else GL_SGI_index_array_formats:=False;
  if Pos('GL_SGI_index_func',Buffer) > 0 then GL_SGI_index_func:=True
                                         else GL_SGI_index_func:=False;
  if Pos('GL_SGI_index_material',Buffer) > 0 then GL_SGI_index_material:=True
                                             else GL_SGI_index_material:=False;
  if Pos('GL_SGI_index_texture',Buffer) > 0 then GL_SGI_index_texture:=True
                                            else GL_SGI_index_texture:=False;
  if Pos('GL_WIN_swap_hint',Buffer) > 0 then GL_WIN_swap_hint:=True
                                        else GL_WIN_swap_hint:=False;
  if Pos('GL_EXT_blend_color',Buffer) > 0 then GL_EXT_blend_color:=True
                                          else GL_EXT_blend_color:=False;
  if Pos('GL_EXT_blend_logic_op',Buffer) > 0 then GL_EXT_blend_logic_op:=True
                                             else GL_EXT_blend_logic_op:=False;
  if Pos('GL_EXT_blend_minmax',Buffer) > 0 then GL_EXT_blend_minmax:=True
                                           else GL_EXT_blend_minmax:=False;
  if Pos('GL_EXT_blend_subtract',Buffer) > 0 then GL_EXT_blend_subtract:=True
                                             else GL_EXT_blend_subtract:=False;
  if Pos('GL_EXT_convolution',Buffer) > 0 then GL_EXT_convolution:=True
                                          else GL_EXT_convolution:=False;
  if Pos('GL_EXT_copy_texture',Buffer) > 0 then GL_EXT_copy_texture:=True
                                           else GL_EXT_copy_texture:=False;
  if Pos('GL_EXT_histogram',Buffer) > 0 then GL_EXT_histogram:=True
                                        else GL_EXT_histogram:=False;
  if Pos('GL_EXT_polygon_offset',Buffer) > 0 then GL_EXT_polygon_offset:=True
                                             else GL_EXT_polygon_offset:=False;
  if Pos('GL_EXT_subtexture',Buffer) > 0 then GL_EXT_subtexture:=True
                                         else GL_EXT_subtexture:=False;
  if Pos('GL_EXT_texture',Buffer) > 0 then GL_EXT_texture:=True
                                      else GL_EXT_texture:=False;
  if Pos('GL_EXT_texture_object',Buffer) > 0 then GL_EXT_texture_object:=True
                                             else GL_EXT_texture_object:=False;
  if Pos('GL_EXT_texture3D',Buffer) > 0 then GL_EXT_texture3D:=True
                                        else GL_EXT_texture3D:=False;
  if Pos('GL_EXT_cmyka',Buffer) > 0 then GL_EXT_cmyka:=True
                                    else GL_EXT_cmyka:=False;
  if Pos('GL_EXT_rescale_normal',Buffer) > 0 then GL_EXT_rescale_normal:=True
                                             else GL_EXT_rescale_normal:=False;
  if Pos('GL_SGI_color_matrix',Buffer) > 0 then GL_SGI_color_matrix:=True
                                           else GL_SGI_color_matrix:=False;
  if Pos('GL_SGI_texture_color_table',Buffer) > 0 then GL_SGI_texture_color_table:=True
                                                  else GL_SGI_texture_color_table:=False;
  if Pos('GL_SGI_color_table',Buffer) > 0 then GL_SGI_color_table:=True
                                          else GL_SGI_color_table:=False;
  if Pos('GL_EXT_clip_volume_hint',Buffer) > 0 then GL_EXT_clip_volume_hint:=True
                                               else GL_EXT_clip_volume_hint:=False;
  if Pos('GL_EXT_misc_attribute',Buffer) > 0 then GL_EXT_misc_attribute:=True
                                             else GL_EXT_misc_attribute:=False;
  if Pos('GL_EXT_scene_marker',Buffer) > 0 then GL_EXT_scene_marker:=True
                                           else GL_EXT_scene_marker:=False;
  if Pos('GL_EXT_shared_texture_palette',Buffer) > 0 then GL_EXT_shared_texture_palette:=True
                                                     else GL_EXT_shared_texture_palette:=False;

  // -- second GLU
  Buffer:=StrPas(PChar(gluGetString(GLU_EXTENSIONS)));
  if Pos('GLU_EXT_TEXTURE',Buffer) > 0 then GLU_EXT_TEXTURE:=True
                                       else GLU_EXT_TEXTURE:=False;
  if Pos('GLU_EXT_object_space_tess',Buffer) > 0 then GLU_EXT_object_space_tess:=True
                                                 else GLU_EXT_object_space_tess:=False;
  if Pos('GLU_EXT_nurbs_tessellator',Buffer) > 0 then GLU_EXT_nurbs_tessellator:=True
                                                 else GLU_EXT_nurbs_tessellator:=False;
end;
...
begin
  ...
  begin
    ...
    if GetObjectType(DC) = OBJ_MEMDC then dwFlags:=dwFlags or PFD_DRAW_TO_BITMAP
...
begin
  ...
  begin
    Result:=InitOpenGLFromLibrary('OpenGL.DLL','GLU.DLL');
    if not Result then Result:=InitOpenGLFromLibrary('OpenGL32.DLL','GLU32.DLL');
  end
  else Result:=True;
end;
