unit TPCAResultsObject;
//{$define FREEPASCAL=1}
{$ifdef FREEPASCAL}
{$mode delphi}
{$endif}
interface   //
uses  classes, SysUtils, TMatrixObject, TMatrixOperations, TVarAndCoVarOperations, TASMTimerUnit  ;
type
  TPCA = class
  public
    dataCreated, dataSaved : boolean ;

//    XMatrix                             : TMatrix ;   // this is (copy of) the input data
...
//    ScoresNormalised, EVectNormalsied   : TMatrix ;   // these are created when CreateModelMatrix() is called. They use ReturnNormalisedEVects()
    EigenValues                         : TMatrix ;   // eigenvalue = diagonals of (scores(PCx)' * scores(PCx))//    RegenMatrix                         : TMatrix ;
//    XResiduals                          : TMatrix ;
    ...
    SDPrecPCA : integer ;  // either 4 or 8 for single or double precision respectively
    ...
    procedure MakeAllComplex ;                                           // not properly implemented
...
uses BLASLAPACKfreePas, FileInfo ;
...
//  XResiduals.ClearData(tSOrD) ;;
  ...
  Scores.ClearData(SDPrecPCA) ;
  EVects.ClearData(SDPrecPCA) ;
//  ScoresNormalised.ClearData(tSOrD) ;
//  EVectNormalsied.ClearData(tSOrD) ;
  EigenValues.ClearData(SDPrecPCA) ;
...
procedure TPCA.MakeAllComplex ; // ???not sure this is consitent with rest of code???
begin
...
{$ifdef test} writeln('Got to GetMaxSumSquaresVector()') ; {$endif}
     ...
     pMKL := inputMatrix.F_Mdata.Memory ;
       ...
       begin
         ...
         begin
{$ifdef test} writeln('sdot() is next -') ; {$endif}
           s1 := sdot ( tnumRows, pMKL, tnumCols, pMKL, tnumCols ) ;
           ss_vector.Write(s1,inputMatrix.SDPrec) ;
{$ifdef test} writeln('MovePointer() is next') ; {$endif}
...
{$ifdef test} writeln('Got past first sdot') ; {$endif}
...
{$ifdef test} writeln('Got past first isamax') ;  {$endif}
...
{$ifdef test} writeln('Got past first scopy') ; {$endif}
     FreeMem(p1) ;
     ss_vector.Free ;
end ;
...
begin
     ...
     MKLbetad := 0.0 ;
     PCs_sofar := 1 ;
     tscoresVect   := TMatrix.Create2(inputMatrix.SDPrec, 1, inputMatrix.numRows) ;
     tEVectsVect   := TMatrix.Create2(inputMatrix.SDPrec, 1, inputMatrix.numCols)  ;

     // this is a copy of the oriinal data, to be used for display - added recently
//     XMatrix  := TMatrix.Create(inputMatrix.SDPrec) ;
...
{$ifdef test} writeln('Got to start of PCA') ;  {$endif}
...
{$ifdef test}     tTimer := TASMTimer.Create(0) ;  {$endif}
     ...
     begin
       ...
       if inputMatrix.SDPrec = 4 then                      // single precision
        ...
        while  (abs(diff_x_SS) > 0.0000001)  and (numIter < 3000)  do   // start of iteratve loop
        begin
           if inputMatrix.SDPrec = 4 then    // single precision
           begin
              MKLtrans := 'n' ;
...
{$ifdef test} tTimer.setTimeDifSecUpdateT1 ;   {$endif}
...
{$ifdef test} tTimer.setTimeDifSecUpdateT1 ;  {$endif}
{$ifdef test} writeln('evector multiply: ' + floattostrf(tTimer.getRecordedTime,ffGeneral,5,4)) ;  {$endif}
...
{$ifdef test}tTimer.setTimeDifSecUpdateT1 ; {$endif}
{$ifdef test} writeln('scores multiply: ' + floattostrf(tTimer.getRecordedTime,ffGeneral,5,4)) ; {$endif}
...
//              diff_x_SS   := 0.0000001 ;
        ...
        Form4.StatusBar1.Panels[1].Text := 'PC number '+inttostr(PCs_sofar)+' of '+inttostr(self.numPCs) +' found. '+inttostr(numIter) + ' of 3000' ;
...
begin
     ...
     tXMatrix := TMatrix.Create2(inputMatrix.SDPrec, 1, inputMatrix.numRows) ;
     tscoresVect   := TMatrix.Create2(inputMatrix.SDPrec, 1, inputMatrix.numRows) ;
     tscoresVect.MakeComplex(tXMatrix) ;
     tXMatrix.Free ;
     tXMatrix := TMatrix.Create2(inputMatrix.SDPrec, 1, inputMatrix.numCols)  ;
     tEVectsVect := TMatrix.Create2(inputMatrix.SDPrec, 1, inputMatrix.numCols)  ;
     ...
     // copy preprocessed data for calculation of scores (projecting the 'latent variables') at end of procedure
     tXMatrix := TMatrix.Create(inputMatrix.SDPrec) ;
     tXMatrix.CopyMatrix(inputMatrix) ;
...
//    eValStrm.CopyFrom(Eigenvalues.F_Mdata,0) ;
    ...
    sqrtEval := TMatrix.Create2(EigenValues.SDPrec, EigenValues.NumCols,  EigenValues.NumCols) ;
...
//  regen     := TMatrix.Create ;
  tScores   := TMatrix.Create(Scores.SDPrec) ;
  tEVects := TMatrix.Create(Scores.SDPrec) ;
