...
type
  ...
  public
    ...
    procedure CreatePLSModelMatrix(xDataIn : TMatrix; yDataIn : TMatrix;  numPCsWanted : integer; IsImageData : boolean) ;  // creates "RegressionCoef" TMatrix for a set of X data with known Y values
...
uses  AtlusBLASLAPACLibrary, TASMTimerUnit ;
...
function PLSResults.ClearResultsData(singleOrDouble : integer; askIfWantToSave : boolean; messageStr : string) : boolean ;
Var
  msgRes : Word ;
begin
...
// pcRange is '1-numPCs'
procedure PLSResults.CreatePLSModelMatrix(xDataIn : TMatrix; yDataIn : TMatrix; numPCsWanted : integer; IsImageData : boolean)  ;
var
  t1, t2 : integer ;
  s1, s2, s3 : single ;
  d1, d2, d3 : double ;
...
begin
try
  tEVects_x     := TMatrix.Create2(xDataIn.SDPrec div 4, 1, xDataIn.numCols) ;
  tScores       := TMatrix.Create2(xDataIn.SDPrec div 4, xDataIn.numRows, 1) ;
  tWeights      := TMatrix.Create2(xDataIn.SDPrec div 4, 1, xDataIn.numCols) ;
  y_sum         := TMemoryStream.Create ; // this is needed for GetTotalRowsColsFromString function to determine numPCsWanted

  tYMatrix := TMatrix.Create(yDataIn.SDPrec div 4) ; // this is used in calculation, and results in residuals
  tYMatrix.CopyMatrix(yDataIn) ;
//tYMatrix.SaveMatrixDataTxt('tYMatrix_PC'+'.txt','        ') ;
  // **** THIS IS PROBABLY NOT NEEDED *** JUST POINT to xDataIn  ****
  YMatrix := TMatrix.Create(yDataIn.SDPrec div 4) ;   // this stores the original Y data
  YMatrix.CopyMatrix(yDataIn) ;

  // copy data for calculation below
//  tXMatrix := TMatrix.Create(xDataIn.SDPrec div 4) ; //1 // this is used in calculation, and results in residuals
//  tXMatrix.CopyMatrix(xDataIn) ;                     //1 // IT IS DELETED IN FINALY BLOCK BELOW
  tXMatrix :=  xDataIn ;    //1
  // **** THIS IS PROBABLY NOT NEEDED *** JUST POINT to xDataIn  ****
//1  XMatrix  := TMatrix.Create(xDataIn.SDPrec div 4) ; //1 // this is a copy of the oriinal data, to be used for display
...
//tXMatrix.SaveMatrixDataTxt('tXMatrix_PC'+'.txt','        ') ;
  ...
  PredictedYPLS := TMatrix.Create2(yDataIn.SDPrec div 4,yDataIn.numRows,1) ;
  PredictedYPLS.Zero(PredictedYPLS.F_Mdata) ; // zero as first column is predicted mean centred data
  R_sqrd_PLS    := TMatrix.Create2(yDataIn.SDPrec div 4, 1, numPCsWanted )  ;
...
//  CalcResidualData ( true ) ;
  ...
  tMat1 := TMatrix.Create(xDataIn.SDPrec div 4) ;
...
//  regen     := TMatrix.Create ;
  tScores   := TMatrix.Create(ScoresPLS.SDPrec div 4) ;
  tEVects := TMatrix.Create(ScoresPLS.SDPrec div 4) ;
