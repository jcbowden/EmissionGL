...
type
  TAutoProjectLoadings  = class(TBatchMVA)
  public
     numPCs : integer ;
     factorPCs  : string ;
...
constructor TAutoProjectLoadings.Create(SDPrec : integer) ;
begin
...
destructor  TAutoProjectLoadings.Destroy; // override;
begin
  tPCAnalysis.Free ;
...
procedure TAutoProjectLoadings.Free;
begin
...
function  TAutoProjectLoadings.GetPCABatchArguments(lineNum, iter : integer; tStrList : TStringList ) : string ; // get input data from memo1 text. Returns final line number of String list
...
{File input is of this format:
   Memo1.Lines.Add('type = Auto Project Loadings') ;
   ...
   //messagedlg('the analysis type is: ' + lowerCase(trim(copy(tstr1,pos('=',tstr1)+1,length(tstr1)-length(copy(tstr1,1,pos('=',tstr1)))))) ,mtinformation,[mbOK],0) ;
           ...
           try
            ...
            // **********************  PCs for factor match ******************************************
             ...
             // ********************** Mean Centre ******************************************
             repeat        // this has to be false for Auto Project Loadings
               inc(lineNum) ;
               tstr1 := bB.GetStringFromStrList(tStrList, lineNum) ;
             until (trim(tstr1) <> '') or (linenum > tStrList.Count)  ;
             ...
             if bB.LeftSideOfEqual(tstr1) = 'column standardise' then
              ...
              // ********************** normalised output of scores and loadings ******************************************
             ...
             if bB.LeftSideOfEqual(tstr1) = 'normalised output' then
                if bB.RightSideOfEqual(tstr1) = 'true'  then
                   normalisedLoadings := true
                else
                   normalisedLoadings :=  false ;
         ...
         begin
           result := inttostr(lineNum) + ', AutoProjectLoadings batch input conversion error' + #13 ;
         end ;
         end ;
...
function TAutoProjectLoadings.GetAllXData( interleavedOrBlocked : integer; sourceSpecRange : TSpectraRanges ) : boolean ;
var
  t2 : integer ;
...
begin
  ...
  begin
      ...
      begin
        MessageDlg('TAutoProjectLoadings.GetAllXData() Error...'#13'x var range values not appropriate: '+xVarRange,mtError, [mbOK], 0)  ;
        result := false ;
        exit ;
      end ;
  end ;
...
// has to be called from external code which has access to the source data (sourceSpecRange)
function TAutoProjectLoadings.GetFactorData( interleavedOrBlocked : integer; sourceSpecRange : TSpectraRanges ) : boolean ;
var
  t2 : integer ;
// TBatch virtual method
// sourceMatrix is either interleaved (interleavedOrBlocked =1) or block structure (interleavedOrBlocked =2)
...
procedure  TAutoProjectLoadings.MinimiseDifference( PC_Spectra, factSpect : TSpectraRanges) ;
var
  t1 : integer ;
...
function TAutoProjectLoadings.SumOfSquaresOfDifferences( value: single ;A_sub_B, COPY_B, Factor : TMatrix) : single ;
...
function  TAutoProjectLoadings.MultAndSub : boolean ; // returns success or failure (true / fasle)
var
  tScores: TSpectraRanges ;
  MKLbetas : single ;
  MKLloadings, MKLscores, MKLdata : pointer ;
  MKLtint, MKLlda : integer ;
begin
    try
      result := false ;

      // create scores by projection of loadings onto original data
      ...
      MKLloadings := MinimisedPCs.yCoord.F_Mdata.Memory ;
      ...
      MKLtint     :=  1 ;
      MKLbetas    := -1.0 ;
      sger (AllXData.yCoord.numCols , AllXData.yCoord.numRows, MKLbetas, MKLloadings, MKLtint, MKLscores , MKLtint, MKLdata, MKLlda) ;
...
function  TAutoProjectLoadings.ProcessData( lc : pointer ) : boolean ;
// factorSpectra and AllXData must be filled before this function is called using 'GatAllXData' and 'GetFactorData'
var
  t1 : integer ;
  s1 : single ;
...
//      tPCAnalysis.XMatrix.CopyMatrix( allXData.yCoord )  ;  // XMatrix copy is done in the tPCAnalysis.PCA() function
    ...
    // get desired PCs from tPCAnalysis Loadings matrix
    tPCAnalysis.Loadings.F_Mdata.Seek(0,soFromBeginning) ;
    PC_AandB.yCoord.FetchDataFromTMatrix( factorPCs, factorVarStr , tPCAnalysis.LoadingNormalsied ) ;
    ...
    if normalisedLoadings then
    begin
      scoresSpectra.yCoord.CopyMatrix( tPCAnalysis.ScoresNormalised ) ;
      eigenVSpectra.yCoord.CopyMatrix( tPCAnalysis.LoadingNormalsied ) ;
    end
    else  // not normalisedLoadings
    begin
      scoresSpectra.yCoord.CopyMatrix( tPCAnalysis.Scores ) ;
      eigenVSpectra.yCoord.CopyMatrix( tPCAnalysis.Loadings ) ;
    end ;

    XresidualsSpectra.yCoord.CopyMatrix( tPCAnalysis.XResiduals )   ;
    eigenValSpectra.yCoord.CopyMatrix( tPCAnalysis.EigenValues ) ;
//    tPCAnalysis.RegenerateData( '1-'+inttostr(numPCs),'1-'+inttostr(tPCAnalysis.Loadings.numCols),meanCentre,tPCAnalysis.XResiduals.F_MAverage) ;  // this is done in PCAResultsObject code
