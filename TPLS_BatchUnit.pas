...
uses
  ...
  TMatrixObject, TMatrixOperations, TPLS_ResultsObjects, TPreprocessUnit ;

type
  TPLSBatch  = class  // does not inherit from TBatchMVA
  public
     numPCs : integer ;
...
//     regenSpectra            : TSpectraRanges ;  // this is inherited // Data made from specific PCs using  RegenerateData() function
  ...
  private
     ...
     function  CreatePLSDisplayData  : boolean ;   // lc is pointer to TGLLineColor type, holding the line colour TSpectraRange.LineColor
end;

implementation


constructor TPLSBatch.Create(SDPrec : integer);  // need SDPrec
begin
...
destructor  TPLSBatch.Destroy; // override;
begin
...
function  TPLSBatch.GetBatchArguments( lineNum, iter : integer; tStrList : TStringList ) : string   ;
 ...
 {       if pos(' ',trim(xVarRange)) > 0 then
       ...
       // ********************** vector normalise ******************************************
        ...
        if bB.LeftSideOfEqual(tstr1) = 'normalise x' then
          normaliseXPLS :=  bB.RightSideOfEqual(tstr1) ;

//        if (normaliseXPLS <> 'vn') or (normaliseXPLS <> '') then
...
function  TPLSBatch.GetAllXData( interleavedOrBlocked : integer;  sourceSpecRange : TSpectraRanges ) : string  ; // TBatch virtual method
var
  t2 : integer ;
...
function  TPLSBatch.GetAllYData( interleavedOrBlocked : integer;  sourceSpecRange : TSpectraRanges ) : string  ; // TBatch virtual method
var
  t1, t2 : integer ;
  s1 : single ;
  d1 : double ;
...
function  TPLSBatch.PreProcessData : string ;
...
begin
     ...
     if pos('pre',normaliseXPLS) > 0 then
       allXData.yCoord.VectorNormaliseRowVects  ;

     self.allYData.yCoord.MeanCentre ; // Y data has to be mean centred (i think)
     if  meanCentre then
        self.allXData.yCoord.MeanCentre ;

     if pos('post',normaliseXPLS) > 0 then
         allXData.yCoord.VectorNormaliseRowVects  ;
...
function  TPLSBatch.DetermineNumPCs : string ;
var
  t1 : integer ;
...
begin
     ...
     begin
       writeln('ProcessPLSBatchFile failed:' + #13 + resultStringIn ) ;
       halt ;
     end ;
end ;
...
//
function  TPLSBatch.ProcessPLSBatchFile(inputX, inputY : TSpectraRanges)  : string ;  // this calls all above functions and creates model
...
function  TPLSBatch.CreatePLSDisplayData  : boolean ;
var
 t1 : integer ;
 s1 : single ;
 d1 : double ;
begin
    ...
    scoresSpectra    := TSpectraRanges.Create(self.allXData.yCoord.SDPrec,  allXData.yCoord.numRows , numPCs                  )  ;
    eigenVSpectra  := TSpectraRanges.Create(self.allXData.yCoord.SDPrec,  numPCs                  ,  allXData.yCoord.numCols)  ;
//1    XresidualsSpectra := TSpectraRanges.Create(self.allXData.yCoord.SDPrec, allXData.yCoord.numRows ,  allXData.yCoord.numCols, lc )  ; //1
    YresidualsSpectra := TSpectraRanges.Create(self.allXData.yCoord.SDPrec,allXData.yCoord.numRows                   ,  numPCs )  ; // numPCs by number of Samples
    eigenValSpectra  := TSpectraRanges.Create(self.allXData.yCoord.SDPrec,  1                       ,  numPcs                 )  ;
//1    regenSpectra     := TSpectraRanges.Create(self.allXData.yCoord.SDPrec,  allXData.yCoord.numRows ,  allXData.yCoord.numCols, lc )  ; //1

    weightsSpectra   := TSpectraRanges.Create(self.allXData.yCoord.SDPrec,  numPCs                  , allXData.yCoord.numCols )  ;
    regresCoefSpectra:= TSpectraRanges.Create(self.allXData.yCoord.SDPrec,  numPCs                  , allXData.yCoord.numCols )  ;
    predYPLSSpectra  := TSpectraRanges.Create(self.allXData.yCoord.SDPrec,  numPCs                  , allXData.yCoord.numRows )  ;
    R_sqrd_PLS       := TSpectraRanges.Create(self.allXData.yCoord.SDPrec,  1                       , numPCs                  )  ;
...
procedure TPLSBatch.SaveSpectra(filenameIn : string)  ;
var
 t1 : integer ;
begin
    t1 := pos('.', filenameIn)-1 ;
